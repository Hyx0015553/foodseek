	...
	export default {
		...
		onLoad(params) {
			listj({params: {table: 'user' }}).then(userlist => {
				let tgids = "";
				let sql = "";
				let favs = this.userInfo.favs2||"";//获取自己喜欢的收藏的数据
				...
				for(let i=0;i<userlist.length;i++){//遍历用户数据
					...
					let ufavs = user.favs2;//别人的收藏喜欢的东西
				...
				sql = "select * from fs_blog order by id desc";
				if (tgids) {
					sql = "select * from fs_blog where id in ("+tgids+")";
				}

				if(tags){
					sql = "select * from fs_blog where (id in ("+tgids+") or typeid in ("+tags+"))";
				}
