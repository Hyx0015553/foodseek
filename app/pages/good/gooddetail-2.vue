    ...
    <scroll-view scroll-y class="main-scroll" :enable-flex="true">
      ...
      <view class="section-container review-section">
        ...
        <view v-if="replaylist.length" class="review-tabs">
          <view
            class="rt-item"
            :class="{ active: reviewTab === 'all' }"
            @tap="reviewTab = 'all'"
          >全部</view>
          <view
            class="rt-item"
            :class="{ active: reviewTab === 'good' }"
            @tap="reviewTab = 'good'"
          >好评 <text class="rt-count">{{ reviewCountGood }}</text></view>
          <view
            class="rt-item"
            :class="{ active: reviewTab === 'neutral' }"
            @tap="reviewTab = 'neutral'"
          >中评 <text class="rt-count">{{ reviewCountNeutral }}</text></view>
          <view
            class="rt-item"
            :class="{ active: reviewTab === 'bad' }"
            @tap="reviewTab = 'bad'"
          >差评 <text class="rt-count">{{ reviewCountBad }}</text></view>
        </view>
        ...
        <scroll-view v-if="replaylist.length" scroll-x class="review-sub-scroll" :show-scrollbar="false" :enable-flex="true">
          <view class="review-sub-inner">
            ...
            <view
              class="rs-chip"
              :class="{ on: filterMedia }"
              @tap="toggleFilterMedia"
            >图/视频 <text class="rs-count">{{ reviewCountWithImg }}</text></view>
          </view>
        </scroll-view>
...
export default {
  ...
  methods: {
    ...
    replay() {
      if (!this.rnote) {
        uni.showToast({ title: '请输入评价内容', icon: 'none' }); return;
      }
      let fdata = {
        table: "replay", pid: this.fobj.id, note: this.rnote,
        uid: this.userInfo.id, username: this.userInfo.username,
        type: 1, pf: this.pingfen, img: this.rimg
      }
      savej({ params: fdata }).then(res => {
        this.rnote = ''; this.rimg = ''; this.pingfen = 5;
        this.listReplay(); this.showpl = false;
    ...
    setPf() {
      listj({ params: { table: 'replay', type: 1, pid: this.fobj.id } }).then(res => {
        ...
        let avgpf = (res.reduce((a, b) => a + b.pf * 1, 0) / res.length).toFixed(1);
        this.fobj.pf = avgpf;
...
.review-form {
  ...
  .form-title { font-weight: bold; margin-bottom: 30rpx; font-size: 30rpx; }
  .form-item {
    margin-bottom: 30rpx;
  ...
  .form-btns {
    ...
    .btn-cancel {
      ...
      background: #f0f2f5; color: #666; font-size: 28rpx;
    }
    .btn-submit {
      flex: 2; text-align: center; padding: 20rpx; border-radius: 12rpx;
      background: #ff943c; color: #fff; font-size: 28rpx;
    }
  }
}
