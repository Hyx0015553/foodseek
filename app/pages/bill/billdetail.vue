  ...
  <view class="page-container">
    ...
    <scroll-view scroll-y class="sv-container">
      ...
      <view class="pickup-card">
        ...
        <view class="tip">下拉可刷新状态，取餐时请向店员出示取餐号</view>
      </view>
      ...
      <view class="card goods-card">
        ...
        <view class="good-item" v-for="(item, index) in fobjgoodlist" :key="index" @click="toGoodDetail(item.id, 1)">
    ...
    <view class="footer-actions">
      <u-button
        v-if="fobj.statecn === '待付款'"
        type="error"
        plain
        size="small"
        text="取消订单"
        @click="changeBillStatecn('已取消')"
        customStyle="width: 180rpx; margin-right: 20rpx;"
      ></u-button>
      <u-button
...
export default {
  ...
  onLoad(params) {
    ...
    // 兼容“直接进入订单详情页”场景：自动加载最近一笔订单
    if (!this.userInfo || !this.userInfo.id) {
      uni.showToast({ title: '请先登录', icon: 'none' })
      setTimeout(() => uni.ytool.toLogin(), 500)
      return
    }
    listj({ params: { table: 'bill', uid: this.userInfo.id, sort: 'id', order: 'desc' } })
      .then(res => {
        const arr = res || []
        if (!arr.length) {
          uni.showToast({ title: '暂无订单信息', icon: 'none' })
          setTimeout(() => uni.switchTab({ url: '/pages/bill/bill' }), 700)
          return
        }
        this.loadDetail(arr[0].id)
      })
      .catch(() => {
  ...
  methods: {
    loadDetail(bid) {
      findj({ params: { table: 'bill', id: bid } }).then(res => {
        this.fobj = res || {}
        // 如果订单使用了优惠券，加载优惠券信息
        if (this.fobj.yhqid) {
        ...
        const gids = (this.fobj.gids || '').toString()
        if (!gids) {
          this.fobjgoodlist = []
          return
        }
        const sql = `select * from fs_good where id in (${gids})`
        listSqlj({ params: { sql } }).then(glist => {
...
.pickup-head {
  ...
  align-items: center;
  .small { color: #999; font-size: 24rpx; }
  .tag { color: #e11d48; font-size: 24rpx; font-weight: 700; }
}
...
.step {
  ...
  color: #c0c4cc;
  width: 150rpx;
  .dot { font-size: 20rpx; line-height: 1; }
  .label { margin-top: 8rpx; font-size: 22rpx; }
  &.active { color: #303133; font-weight: 700; }
}
...
.shop-name { font-size: 30rpx; font-weight: 700; color: #1f2937; }
.shop-time { font-size: 22rpx; color: #9ca3af; }
...
.good-info {
  flex: 1;
  margin-left: 16rpx;
  .good-name { font-size: 28rpx; color: #111827; }
  .good-desc { margin-top: 6rpx; font-size: 24rpx; color: #9ca3af; }
}
...
.summary-line {
  ...
  font-size: 28rpx;
  color: #374151;
  .sum-price { font-size: 36rpx; font-weight: 800; color: #111827; }
}
